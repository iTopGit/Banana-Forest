...
function tweet_table(blog_data) {
    const data = {
        tweet: blog_data
    };
    ...
    const createTweetHTML = ({
        ...
        if (oldDate.getUTCSeconds() === editDate.getUTCSeconds()) {
            ...
            return edit_Blog(id, name, message, email, date_post, date_edit, date_full,avatar_url);
        }

    };
    const tweet = data.tweet.map(createTweetHTML);
...
function edit_Blog(id, name, message, email, date_post, date_edit, date_full,avatar_url) {
...
function loadDoc(blog_data) {
    document.getElementById("blog-data").innerHTML = "";
    document.getElementById("blog-data").addEventListener("load", tweet_table(blog_data));
}
